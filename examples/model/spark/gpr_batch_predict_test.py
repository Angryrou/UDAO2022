...
import sys
import getopt
#from batch_models import Batch_models
from model.architecture.gpr_batch_models_eliminate import GPR_Batch_models


HELP = """
Format: python gpr_batch_predict_test.py
"""


def main():

    gpr_bm = GPR_Batch_models()
    print('Built GPR model. Ready to make batch predictions for various scenarios...')
    ...
    if choice==1:
        ...
        Y_predicted_batch = gpr_bm.predict(X_predict_latency)
        print(f'Batch predicted value {Y_predicted_batch}\n=======')
        ##
        Y_predicted_global_min = gpr_bm.opt_scenario1(zmesg=X_predict_latency, max_iter=100, lr=0.1)
        ...
        Y_predicted_batch = gpr_bm.predict(X_predict_cpu)
        print(f'Batch predicted value {Y_predicted_batch}\n=======')
        ##
        Y_predicted_global_min = gpr_bm.opt_scenario1(zmesg=X_predict_cpu, max_iter=100, lr=0.1)
        print(f'Predicted "global" minimum for selected objective: {Y_predicted_global_min}\n=======')
        ##

    elif choice==3:
        print(f'Example input for prediction: {X_predict_network}\n=======')
        Y_predicted_batch = gpr_bm.predict(X_predict_network)
        print(f'Batch predicted value {Y_predicted_batch}\n=======')
        ##
        Y_predicted_global_min = gpr_bm.opt_scenario1(zmesg=X_predict_network, max_iter=100, lr=0.1)
        print(f'Predicted "global" minimum for selected objective: {Y_predicted_global_min}\n=======')
        ##
