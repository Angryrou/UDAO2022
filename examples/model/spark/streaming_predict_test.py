...
import sys
import getopt
from model.architecture.streaming_models_eliminate import Streaming_models

HELP = """
Format: python streaming_predict_test.py
"""
...
def main():
    ...
    if choice==1:
        ...
        Y_predicted_streaming = sm.predict(X_predict_latency)
        print(f'Streaming predicted value {Y_predicted_streaming}\n=======')
        ##
        Y_predicted_global_min = sm.opt_scenario1(zmesg=X_predict_latency, max_iter=100, lr=0.1)
        ...
        Y_predicted_streaming = sm.predict(X_predict_cpu)
        print(f'Batch predicted value {Y_predicted_streaming}\n=======')
        ##
        Y_predicted_global_min = sm.opt_scenario1(zmesg=X_predict_cpu, max_iter=100, lr=0.1)
        print(f'Predicted "global" minimum for selected objective: {Y_predicted_global_min}\n=======')
        ##

    elif choice==3:
        print(f'Example input for prediction: {X_predict_network}\n=======')
        Y_predicted_streaming = sm.predict(X_predict_network)
        print(f'Batch predicted value {Y_predicted_streaming}\n=======')
        ##
        Y_predicted_global_min = sm.opt_scenario1(zmesg=X_predict_network, max_iter=100, lr=0.1)
        print(f'Predicted "global" minimum for selected objective: {Y_predicted_global_min}\n=======')
        ##
