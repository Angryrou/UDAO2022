...
if __name__ == '__main__':
    ...
    conf_df_target = conf_df.loc[tid_target]
    total_queries = conf_df_target.shape[0]
    # conf_df_dict = {
    #     tid: conf_df.loc[tid].sample(frac=1, random_state=i)[t_st: t_en]
    #     for i, tid in enumerate(templates) if tid == tid_target
    # }
    ...
    while submit_index < total_queries:
        ...
        if if_submit:
            ...
            # submit(lock, current_cores, cores, tid, qid, knob_sign, debug, script_header, log_header)
            submit_index += 1
