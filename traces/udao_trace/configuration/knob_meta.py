from udao_trace.utils import VarTypes, ScaleTypes
...
class KnobMeta(object):
    ...
    def __init__(self,
        ...
        self.min = kmin
        self.max = kmax
        self.scale = ScaleTypes.LINEAR if scale is None or scale == "linear" else ScaleTypes.LOG
        ...
        if self.ktype in (VarTypes.INT, VarTypes.FLOAT):
            assert self.min is not None and self.max is not None, f"min/max value of {self.name} is not set"
            assert self.min <= self.max, f"min value {self.min} is larger than max value {self.max}"
            assert self.factor is not None, f"factor of {self.name} is not set"
        elif self.ktype == VarTypes.CATEGORY:
            assert self.categories is not None and len(self.categories) >= 1, f"categories of {self.name} is not set"
