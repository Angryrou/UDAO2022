...
class SparkParser:
    ...
    def _parse_conf(conf: Dict) -> Dict:
        d = {f"{k}": v for k_type, k_list in conf.items() for kv in k_list for k, v in kv.items()}
        return {
    ...
    def _parse_base(self, d: Dict) -> (Dict, Dict):
    ...
    def _parse_lqp(self, feat: Dict, obj: Dict, meta: Dict, theta_c: Optional[Dict] = None) -> Dict:
    ...
    def parse_one_file(self, file: str) -> (List, List):
        ...
        for lqp_id, runtime_lqp in d["RuntimeLQPs"].items():
            runtime_lqp_dict = self._parse_lqp(
                runtime_lqp, runtime_lqp["Objectives"], {**meta, **{"lqp_id": lqp_id}}, theta_c)
    ...
    def parse(self, header, templates, upto=10, n_processes=12):
        ...
        logger.info(f"start flat join the results...")
        ...
        df_q.to_csv(f"{csv_header}/q_{len(templates)}x{upto}.csv", index=False, header=True)
        df_qs.to_csv(f"{csv_header}/qs_{len(templates)}x{upto}.csv", index=False, header=True)
        return df_q, df_qs
