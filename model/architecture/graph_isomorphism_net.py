...
class GIN(nn.Module):
    ...
    def __init__(self, net_params):
        ...
        if in_feat_size_op < hidden_dim:
            ...
            layers.append(GINConv(nn.Linear(in_feat_size_op, hidden_dim)))

        for i in range(n_gcn_layers - 1):
            layers.append(GINConv(nn.Linear(hidden_dim, hidden_dim // 2)))
            hidden_dim = hidden_dim // 2

        assert hidden_dim >= out_dim
        layers.append(GINConv(nn.Linear(hidden_dim, out_dim)))
