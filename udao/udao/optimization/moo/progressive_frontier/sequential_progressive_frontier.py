...
class SequentialProgressiveFrontier(BaseProgressiveFrontier):
    ...
    def solve(
        ...
        """
        primary_objective_idx = self._first_float_objective(problem)
        ...
        for _ in range(self.n_probes - n_objs):
            ...
            middle_point, subrectangles = self._find_local_optimum(
                problem, rectangle, primary_objective_idx, seed=seed
            )
    ...
    def _find_local_optimum(
        ...
        so_problem = self._so_problem_from_bounds_dict(
            problem, obj_bounds_dict, problem.objectives[primary_objective_idx]
        )
        try:
            ...
            return middle_point, self.generate_sub_rectangles(
                current_utopia, current_nadir, middle_point, primary_objective_idx
            )
    ...
    def generate_sub_rectangles(
        ...
        for point in corner_points:
            ...
            is_explored_unconclusive = not successful and all(
                (middle.objs[i] - point.objs[i] > 0)
                for i in range(len(point.objs))
                if i != primary_objective_idx
            )
            # nadir point
