from typing import Dict, List

import torch as th
from torchmetrics import Metric

EPS = 1e-7


class ErrorPercentiles(Metric):
    def __init__(
        self,
        quantiles: List[float] = [0.5, 0.9, 0.95, 0.99],
        dist_sync_on_step: bool = False,
    ) -> None:
        super().__init__(dist_sync_on_step=dist_sync_on_step)
        self.quantiles = quantiles
        self.add_state("error_rate", default=th.empty((0, 1)), dist_reduce_fx=None)

    def update(self, input: th.Tensor, target: th.Tensor) -> None:
        error_rate = th.abs(input - target) / (target + EPS)
        self.error_rate.cat(error_rate)  # type: ignore

    def compute(self) -> Dict[str, th.Tensor]:
        error_quantiles = th.quantile(self.error_rate, self.quantiles, dim=0)  # type: ignore
        return {
            "mean": self.error_rate.mean(),  # type: ignore
            **{
                f"q_{q * 100}": error_quantiles[i] for i, q in enumerate(self.quantiles)
            },
        }


class QErrorPercentiles(Metric):
    def __init__(
        self,
        quantiles: List[float] = [0.5, 0.9, 0.95, 0.99],
        dist_sync_on_step: bool = False,
    ) -> None:
        super().__init__(dist_sync_on_step=dist_sync_on_step)
        self.quantiles = quantiles
        self.add_state("q_error", default=th.empty((0, 1)), dist_reduce_fx=None)

    def update(self, input: th.Tensor, target: th.Tensor) -> None:
        q_error = th.maximum(
            (target + EPS) / (input + EPS),
            (input + EPS) / (target + EPS),
        )
        self.q_error.cat(q_error)  # type: ignore

    def compute(self) -> Dict[str, th.Tensor]:
        q_error_quantiles = th.quantile(self.q_error, self.quantiles, dim=0)  # type: ignore
        return {
            "mean": self.q_error.mean(),  # type: ignore
            **{
                f"q_{q * 100}": q_error_quantiles[i]
                for i, q in enumerate(self.quantiles)
            },
        }
