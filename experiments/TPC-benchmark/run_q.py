...
if __name__ == "__main__":
    ...
    data_processor = data_processor_getter(
        ...
        objectives = FeaturePipeline(
            extractor=TabularFeatureExtractor(columns=["latency_s", "io_mb"]),
        ),
    ...
    #     tensor_dtypes=tensor_dtypes,
    #     tabular_features=FeaturePipeline(
    #         extractor=TabularFeatureExtractor(columns=variable_names + LQP_NON_DECISION_VARS),
    #         preprocessors=[
    #             NormalizePreprocessor(MinMaxScaler())
    #         ],
    #     ),
    #     objectives=FeaturePipeline(
    #         extractor=TabularFeatureExtractor(columns = ["latency", "io"]),
    #     ),
    #     query_structure=FeaturePipeline(
    #         extractor=LQPExtractor(positional_encoding_size=10),
    #         preprocessors=[
