...
```bash
# bash examples/trace/spark/1.query_generation_tpch.sh <tpch-kit path> <benchmark-out path> <#queries per template> <SF-100 by default>
# TPCH
# local:
...
-b TPCH \
--workload-header "resources/tpch-kit/spark-sqls" \
--num-templates 22 \
...
python examples/trace/spark/8.run_all_pressure_bo.py \
-b TPCDS \
--workload-header "resources/tpcds-kit/spark-sqls" \
--num-templates 103 \
