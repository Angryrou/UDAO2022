...
```bash
   ...
   # running 30 template queries in the multi-benchmark env
   # (1) configuration generation
   python examples/trace/spark/5.generate_scripts_for_lhs_tpcxbb.py -b TPCxBB --num-processes 30 --num-templates 30
   # (2) running for debug 
   python examples/trace/spark/6.run_all_pressure_test_tpcxbb.py -b TPCxBB --id-template-end 1428 --debug 1 --worker debug
   python examples/trace/spark/6.run_all_pressure_test_tpcxbb_makeup.py -b TPCxBB --debug 1 --worker debug
  ...
  python examples/trace/spark/6.run_all_pressure_test_tpcxbb_makeup.py \
  -b TPCxBB \
  --debug 0 \
  --num-processes 22 \
  --worker hex3
  
  python examples/trace/spark/7.dump_lhs_results_tpcxbb.py \
  -b TPCxBB \
  --script-header resources/scripts/tpcxbb-lhs \
  --num-templates 30 \
  --log-header examples/trace/spark/6.run_all_pressure_test_tpcxbb/log
  
  python examples/trace/spark/8.run_all_pressure_bo_tpcxbb.py --debug 0 --worker hex3 --num-processes 22
```
